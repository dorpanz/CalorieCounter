...
const Dinner = () =>{
    ...
    const deleteFoodItem = (index) => {
        const updatedDinner = [...mealTimes.dinner];
        updatedDinner.splice(index, 1);
        setMealTimes({...mealTimes, dinner: updatedDinner})
    };
    const clearAllFoods = () =>{
        setMealTimes({...mealTimes, dinner: []})
    };
    ...
    const updateFoodQuantity = (index, quantity) => {
        const updatedDinner = [...mealTimes.dinner];
        updatedDinner[index].quantity = quantity;
        setMealTimes({ ...mealTimes, dinner: updatedDinner });
    };
    ...
    const handleAddFood = (food , nutrients)=>{
        if (isAdding) return;
        setIsAdding(true);
        addFoodToMeal({text : food, nutrients}, 'dinner');
        setShowSearchForm(true);
        ...
        <div className="meal summary">
            ...
            <div className="added-foods">
                ...
                {mealTimes.dinner.length === 0 ? (
                    ...
                    <ul>
                        {mealTimes.dinner.map((food, index) => (
